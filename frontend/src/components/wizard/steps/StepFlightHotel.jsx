...
import { CheckCircle2, PlaneTakeoff, ArrowLeft, Compass, Info, PenLine, ListChecks } from "lucide-react";
...
export default function StepFlightHotel() {
  ...
  const { destination, travelGroupType, loading, error } = useWizardState();
  const [hasBooking, setHasBooking] = useState(null);
  const [phase, setPhase] = useState("dates"); // dates | hotel
  const [dates, setDates] = useState({ departureDate: "", returnDate: "" });
  ...
  // phase === "hotel"
  const canBuild = hotelMode === "browse" ? Boolean(selectedHotel) : Boolean(manualHotel.hotelName.trim());
  ...
  return (
    <div>
      ...
      <div className="wizard-step__actions">
        ...
        <button type="button" className="btn btn-primary" disabled={loading || !canBuild} onClick={handleBuildItinerary}>
